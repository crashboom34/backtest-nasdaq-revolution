...
- [ ] `app.py` : afficher les jobs `results/job_xxx/` dans l'interface Streamlit (actuellement seul `optimization_history/` est listé)
...
|---------------------------------------|-------------|-----------------------------------------------------------|
| `app.py` ne liste pas les jobs CLI    | En attente  | Deferred volontairement, à faire après validation CLI      |
