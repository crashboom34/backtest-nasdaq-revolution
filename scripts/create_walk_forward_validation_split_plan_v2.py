"""
scripts/create_walk_forward_validation_split_plan_v2.py — AF-V-02 Slice 7, ADR 0021 Décision 8.

Mirroring exact de scripts/create_walk_forward_validation_split_plan.py (Slice 1, v1) — v1 reste
intact, jamais modifié, jamais réimporté ici (chaque script reste autonome, Rule of Three pas
atteinte avec seulement 2 occurrences). `DatasetSplitPlan` est immuable
(`save_dataset_split_plan()` refuse un `split_plan_id` déjà écrit) et le plan historique de Perfect
Revolution (`results/dataset_splits/split_perfect_revolution_v1_final_holdout/split_plan.json`) a
`validation: null` (vérifié, jamais un consommateur réel jusqu'ici — voir ADR 0021 Décision 8). Ce
script construit un **second nouveau** plan, sous un `split_plan_id` distinct à la fois du plan
historique et du plan v1, jamais une modification rétroactive de l'un ou l'autre :
...
**Allocation retenue pour `VALIDATION_MONTHS`** (décision explicite de l'utilisateur, 2026-09-21 —
remplace le défaut raisonné mais jamais confirmé de 42 mois retenu par Slice 1/v1, qui reste
inchangé sous son propre `split_plan_id`) : 54 mois = `train_period` (24) + 5 × `step_period` (6)
du préréglage Rolling par défaut — dimensionnée pour faire tenir EXACTEMENT 5 folds P24M/P6M/P6M
sans reliquat partiel sur CETTE allocation précise. Le mécanisme de détection de reliquat partiel
(`walk_forward.detect_partial_tail()`) reste testé séparément via des fixtures synthétiques,
indépendamment de ce choix — un futur ajustement de cette constante n'affecterait qu'un nouveau
`split_plan_id`, jamais celui déjà écrit (immuabilité).
"""
...
# permet l'exécution standalone (`python scripts/create_walk_forward_validation_split_plan_v2.py`)
...
VALIDATION_MONTHS = 54


def build_walk_forward_validation_split_plan_v2(historical: DatasetSplitPlan) -> DatasetSplitPlan:
    """Fonction PURE : dérive le nouveau plan v2 à partir d'un `DatasetSplitPlan` historique déjà
